geoscale.plot<-function (ages, data, units = c("Age", "Epoch", "Period"), scale = "Period", 
          boxes = "Age", cex.age = 0.3, cex.ts = 0.4, cex.pt = 1, xlim = NULL, 
          ylim = NULL, nscale, ts.col = TRUE, height = 0.3, label = NULL,vers="ICS2013", 
          ...) 
{
...
convert<-matrix(ncol=2,nrow=5)
  ...
  par(mar = c(1, 4, 0, 2))
  plot(ages, data, type = "n", xaxt = "n", yaxt = "n", bty = "n", 
       ylab = "", xlab = "", xlim = xlim)
  vals <- tscale_data[units, "Depth"]
  vals <- c(vals, 0.8)
  vals <- cumsum(vals/sum(vals))
  vals <- c(par()$usr[4], par()$usr[4] - (vals * (par()$usr[4] - 
                                                    par()$usr[3])))
  val <- vals[length(vals) - 1] - vals[length(vals)]
  if (scale != "n") {
    text(time, (vals[length(vals)] + val * 0.3), time, cex = cex.age, 
         srt = 90)
    segments(ticks, (vals[length(vals) - 1]), ticks, (vals[length(vals)] + 
                                                        val * 0.75), lwd = lwd)
  }
  for (t in 1:length(units)) {
    ...
    if (ts.col == TRUE & units[t] != "Other") {
      rect(tscale_n[, "Start"], vals[t], tscale_n[, "End"], 
           vals[t + 1], col = rgb(tscale_n[, "Col_R"], tscale_n[, 
                                                                "Col_G"], tscale_n[, "Col_B"], maxColorValue = 255))
    }
    else rect(tscale_n[, "Start"], vals[t], tscale_n[, "End"], 
              vals[t + 1], col = "white")
    text(tscale_n[, "Midpoint"], (vals[t] + vals[t + 1])/2, 
         tscale_n[, "Name"], cex = cex.ts * tscale_data[match(units[t], 
                                                                rownames(tscale_data)), "size"], srt = tscale_data[match(units[t], 
                                                                                                                         rownames(tscale_data)), "srt"])
  }
  par(fig = c(0, 1, (height), 1), new = T)
  par(mar = c(0, 4, 2, 2))
  plot(ages, data, type = "n", xaxt = "n", yaxt = "n", ann = T, 
       bty = "n", ylab = label, ylim = ylim, xlim = xlim)
  if (!missing(boxes)) {
    if (boxes == "Other") {
      tscale_x <- nscale
    }
